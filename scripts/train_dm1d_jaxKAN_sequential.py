...
flags.DEFINE_integer("n_pde_q", 8192, "time samples in pde batch")
...
def main(argv):
    ...
    sim_data = SimDataset(n_data_points = 8000*8000, n_pde_a=FLAGS.n_pde_a, n_pde_q=FLAGS.n_pde_q,
                          key=FLAGS.datakey, size=size)
    ...
    def compute_loss(model, sample):
        ...
        loss_ic = myloss(zic,sample["ic_z"]) + myloss(zic_t, sample["ic_v"])+ myloss(zic_tt, sample["ic_acc"])
